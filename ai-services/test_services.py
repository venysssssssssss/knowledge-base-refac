...
async def test_service_health(client: httpx.AsyncClient, url: str, name: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"❌ {name} service returned {response.status_code}: {response.text}")
            return False
    except Exception as e:
        print(f"❌ {name} service is not accessible: {e}")
        return False

async def test_rag_query(client: httpx.AsyncClient, rag_url: str, question: str) -> Dict[str, Any]:
    ...
    try:
        ...
        response = await client.post(f"{rag_url}/ask", json=payload, timeout=30.0)
        if response.status_code == 200:
...
async def test_document_search(client: httpx.AsyncClient, doc_processor_url: str, query: str) -> Dict[str, Any]:
...
async def run_tests(rag_url: str, doc_processor_url: str, mistral_url: str):
    ...
    async with httpx.AsyncClient() as client:
        ...
        doc_healthy = await test_service_health(client, doc_processor_url, "Document Processor")
        mistral_healthy = await test_service_health(client, mistral_url, "Mistral")
        ...
        doc_result = await test_document_search(client, doc_processor_url, question)
        
        # Check if search returned any results
        if not doc_result or "chunks" not in doc_result or not doc_result["chunks"]:
            print("⚠️ Document search returned no results, RAG may not work properly")
        ...
        print("\n📊 Diagnostic Summary:")
        print(f"- Document search returned {len(doc_result.get('chunks', []))} chunks")
        if doc_result and "chunks" in doc_result and doc_result["chunks"]:
            print(f"- Top search score: {doc_result['chunks'][0].get('score', 'N/A')}")
        print(f"- RAG response time: {rag_result.get('processing_time', 'N/A')}s")
...
def main():
    ...
    parser.add_argument("--rag", default="http://localhost:8002", help="RAG service URL")
    parser.add_argument("--doc", default="http://localhost:8001", help="Document processor URL")
    parser.add_argument("--mistral", default="http://localhost:8003", help="Mistral service URL")
    parser.add_argument("--question", default="quem pode solicitar alteração cadastral", help="Test question")
